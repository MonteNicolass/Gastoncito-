...
function alertToSuggestion(alert, context) {
  ...
  // MiráPrecios suggestions (price intelligence)
  if (type === 'mira_precios') {
    switch (subtype) {
      case 'precio_alto_vs_historico':
        return {
          id: `suggest_${alert.id}`,
      ...
      case 'timing_recomendado':
        return {
          ...
          domain: 'money',
          text: message,
          subtext: alert.detail || 'Basado en patrones de precios',
          priority: 2,
          actions: [
            {
              label: 'Entendido',
  ...
  // Diet-aware suggestions
  if (type === 'dieta') {
    switch (subtype) {
      case 'alimento_clave_caro':
        return {
          id: `suggest_${alert.id}`,
          alertId: alert.id,
          type: 'warning',
