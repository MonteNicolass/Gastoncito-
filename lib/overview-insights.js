...
export function getMentalOverview(lifeEntries) {
  ...
  // Último registro
  const lastEntry = mentalEntries.length > 0
    ? mentalEntries.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0]
    : null
  ...
  return {
    ...
    lastEntry: lastEntry ? {
      score: lastEntry.meta.mood_score,
...
export function getPhysicalOverview(lifeEntries) {
  ...
  // Días desde último ejercicio
  let daysSinceLastExercise = null
  if (physicalEntries.length > 0) {
    const lastExercise = physicalEntries.sort((a, b) =>
      new Date(b.created_at) - new Date(a.created_at)
    )[0]
    const lastExerciseDate = new Date(lastExercise.created_at)
    lastExerciseDate.setHours(0, 0, 0, 0)
    const todayStart = new Date(now)
    todayStart.setHours(0, 0, 0, 0)
    daysSinceLastExercise = Math.floor((todayStart - lastExerciseDate) / (1000 * 60 * 60 * 24))
  }
