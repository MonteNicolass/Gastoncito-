 ...
 * Usa baseline personal para comparaciones precisas
 */
...
/**
 * Detecta anomalías usando baseline personal
 */
function detectAnomaliesWithBaseline(movimientos, lifeEntries, goals, baseline) {
  const anomalies = []

  // Money: Gasto fuera del rango normal
  if (baseline.money) {
    const today = new Date().toISOString().slice(0, 10)
    const todayMovs = movimientos.filter(m => m.fecha === today && m.tipo === 'gasto')
    const todaySpending = todayMovs.reduce((sum, m) => sum + m.monto, 0)

    if (todaySpending > baseline.money.normalRange.max) {
      const percentAbove = ((todaySpending - baseline.money.avgDaily) / baseline.money.avgDaily) * 100
      const formatAmount = (amount) => {
        return new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 0 }).format(amount)
      }

      anomalies.push({
        type: 'baseline_spending_high',
        severity: percentAbove >= 100 ? 'high' : 'medium',
        title: `Gasto diario +${Math.round(percentAbove)}% vs tu promedio`,
        message: `Hoy: ${formatAmount(todaySpending)}, tu promedio: ${formatAmount(baseline.money.avgDaily)}`,
        data: { today: todaySpending, baseline: baseline.money.avgDaily, percentAbove }
      })
    }
  }

  // Mental: Estado fuera del rango normal
  if (baseline.mental) {
    const last3Days = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000)
    const recentMood = lifeEntries.filter(e =>
      e.domain === 'mental' &&
      e.meta?.mood_score &&
      new Date(e.created_at) >= last3Days
    )

    if (recentMood.length > 0) {
      const belowNormal = recentMood.filter(e => e.meta.mood_score < baseline.mental.normalRange.min)

      if (belowNormal.length >= recentMood.length * 0.6) {
        anomalies.push({
          type: 'baseline_mood_low',
          severity: 'high',
          title: `Estado mental debajo de tu rango normal ${belowNormal.length} días`,
          message: `Tu rango normal: ${baseline.mental.normalRange.min}-${baseline.mental.normalRange.max}/10`,
          data: { belowCount: belowNormal.length, normalRange: baseline.mental.normalRange }
        })
      }
    }
  }

  // Físico: Inactividad vs patrón habitual
  if (baseline.physical) {
    ...
    if (physicalEntries.length > 0) {
      ...
      const daysSince = Math.floor((Date.now() - lastExercise) / (1000 * 60 * 60 * 24))

      if (daysSince > baseline.physical.maxAcceptableGap) {
        anomalies.push({
          type: 'baseline_physical_gap',
          severity: 'medium',
          title: `Actividad física interrumpida vs patrón habitual`,
          message: `${daysSince} días sin ejercicio, tu promedio: ${baseline.physical.avgGapDays} días`,
          data: { daysSince, baseline: baseline.physical.avgGapDays }
        })
      }
    }
  }
...
/**
 * Detecta todas las anomalías (con y sin baseline)
 */
export function detectAllAnomalies(movimientos, lifeEntries, categorias, goals) {
  // Calcular baseline
  const baseline = getBaseline(movimientos, lifeEntries, goals)

  // Anomalías basadas en baseline
  const baselineAnomalies = detectAnomaliesWithBaseline(movimientos, lifeEntries, goals, baseline)

  // Anomalías tradicionales (sin baseline)
  const traditionalAnomalies = [
  ...
  // Combinar (baseline first)
  const allAnomalies = [...baselineAnomalies, ...traditionalAnomalies]
