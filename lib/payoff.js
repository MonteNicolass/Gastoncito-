...
export function generatePayoff(actionType, context = {}) {
  switch (actionType) {
    ...
    case 'price_insight':
      return generatePriceInsightPayoff(context)
...
/**
 * Price insight payoff (MiráPrecios)
 */
function generatePriceInsightPayoff(context) {
  const { isHigh, label, emoji } = context
  ...
  if (isHigh) {
    return {
      ...
      icon: 'TrendingUp',
      color: 'amber',
      message: label || 'Precio por encima de lo habitual',
      detail: null
    }
  }
