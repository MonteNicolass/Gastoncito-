...
export default function CartPage() {
  ...
  const trackedProducts: TrackedProduct[] = useMemo(() => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return trackedProducts.filter((p: TrackedProduct) =>
  ...
  // Cart items already in cart (for filtering in add modal)
            ...
            {analysis.hasEnoughData ? (
              ...
              <Card className="p-6 text-center">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                  ...
                  Registrá compras con supermercado y precio desde el chat.
                </p>
              </Card>
            )}
          </div>
          ...
          <div className="relative w-full max-w-[420px] bg-white dark:bg-zinc-900 rounded-t-3xl animate-slide-up max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-1">
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((p: TrackedProduct) => {
                  const inCart = cartProductIds.has(
                    p.product_name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().replace(/\s+/g, '_')
                  )
                  return (
                    <button
                      key={p.product_name}
