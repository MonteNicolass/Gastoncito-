    ...
    return (
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-24 space-y-4">
          ...
          <div className="space-y-2">
            <Skeleton className="w-16 h-3 rounded-lg" />
            <div className="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50">
          ...
          <>
            ...
            {todayData.mentalEntries.length > 0 && (
              <div className="space-y-2">
                ...
                <Card className="p-4 bg-zinc-50 dark:bg-zinc-900 border-zinc-200/50 dark:border-zinc-700/50 space-y-3">
                  {todayData.mentalEntries.map((entry) => (
                    <div key={entry.id} className="flex items-start justify-between">
                      ...
                      {entry.meta?.mood_score && (
                        <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-100 ml-3">
                          {entry.meta.mood_score}
                        </div>
                      )}
                    </div>
                ...
                <Card className="p-4 space-y-4">
                  {todayData.notes.map((note, idx) => (
                    <div key={note.id} className={idx > 0 ? 'pt-3 border-t border-zinc-100 dark:border-zinc-800' : ''}>
                      {note.type && (
                        <div className="mb-2">
                          <span className="inline-block px-2.5 py-1 text-xs font-semibold rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 capitalize">
                            {note.type}
                          </span>
                        </div>
                      )}
